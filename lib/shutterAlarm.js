        ...
        for (const i in result) {
            for (const s in shutterSettings) {
                if (shutterSettings[s].shutterName == result[i].shutterName) {
                    ...
                    const _alarmWind1 = adapter.config.alarmWind1 != '' ? await adapter.getForeignStateAsync(adapter.config.alarmWind1) : null;
                    if (typeof _alarmWind1 != undefined && _alarmWind1 != null) {
                        statusAlarmWind1 = _alarmWind1.val;
                    }

                    const _alarmWind2 = adapter.config.alarmWind2 != '' ? await adapter.getForeignStateAsync(adapter.config.alarmWind2) : null;
                    if (typeof _alarmWind2 != undefined && _alarmWind2 != null) {
                        statusAlarmWind2 = _alarmWind2.val;
                    }

                    const _alarmRain = adapter.config.alarmRain != '' ? await adapter.getForeignStateAsync(adapter.config.alarmRain) : null;
                    if (typeof _alarmRain != undefined && _alarmRain != null) {
                        statusAlarmRain = _alarmRain.val;
                    }

                    const _alarmFrost = adapter.config.alarmFrost != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFrost) : null;
                    if (typeof _alarmFrost != undefined && _alarmFrost != null) {
                        statusAlarmFrost = _alarmFrost.val;
                    }

                    const _alarmFire = adapter.config.alarmFire != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFire) : null;
...
/**
 * @param {{ log: { debug: (arg0: string) => void; }; config: { alarmWind1: string; alarmWind2: string; alarmRain: string; alarmFire: string; }; getForeignStateAsync: (arg0: any) => any; }} adapter
 * @param {{ enableAlarmWind1: boolean; enableAlarmWind2: boolean; enableAlarmRain: boolean; enableAlarmFire: boolean; enableAlarmFrost: boolean; currentAction: string; name: string; }} Shutter
 */
async function checkPendingAlarm(adapter, Shutter) {
    return new Promise(async (resolve) => {
        ...
        const _statusAlarmWind1 = adapter.config.alarmWind1 != '' ? await adapter.getForeignStateAsync(adapter.config.alarmWind1) : null;
        if (typeof _statusAlarmWind1 != undefined && _statusAlarmWind1 != null && _statusAlarmWind1.val) {
            statusAlarmWind1 = _statusAlarmWind1.val;
        }
        const _statusAlarmWind2 = adapter.config.alarmWind2 != '' ? await adapter.getForeignStateAsync(adapter.config.alarmWind2) : null;
        if (typeof _statusAlarmWind2 != undefined && _statusAlarmWind2 != null && _statusAlarmWind2.val) {
            statusAlarmWind2 = _statusAlarmWind2.val;
        }

        const _statusAlarmRain = adapter.config.alarmRain != '' ? await adapter.getForeignStateAsync(adapter.config.alarmRain) : null;
        if (typeof _statusAlarmRain != undefined && _statusAlarmRain != null && _statusAlarmRain.val) {
            statusAlarmRain = _statusAlarmRain.val;
        }

        const _statusAlarmFire = adapter.config.alarmFire != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFire) : null;
...
/**
 * @param {{ log: { debug: (arg0: string) => void; }; config: { alarmFrost: string; }; getForeignStateAsync: (arg0: any) => any; }} adapter
 * @param {{ enableAlarmWind1: boolean; enableAlarmWind2: boolean; enableAlarmRain: boolean; enableAlarmFire: boolean; enableAlarmFrost: boolean; name: string; }} Shutter
 */
async function checkFrostAlarm(adapter, Shutter) {
    return new Promise(async (resolve) => {
        ...
        const _resAlarmFrost = adapter.config.alarmFrost != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFrost) : null;
