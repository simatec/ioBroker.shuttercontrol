        ...
        for (const i in result) {
            for (const s in shutterSettings) {
                if (shutterSettings[s].shutterName == result[i].shutterName) {
                    ...
                    const pendingAlarm = checkPendingAlarm(adapter, shutterSettings[s]);
                    /**
                    * @param {any} err
...
async function checkPendingAlarm(adapter, Shutter) {
    return new Promise(async (resolve) => {
        ...
        return resAlarmPending; // only temp for old Code ... after code update must delete
    });
}
...
async function checkFrostAlarm(adapter, Shutter) {
    return new Promise(async (resolve) => {
        ...
        return resAlarmFrost; // only temp for old Code ... after code update must delete
    });
}
