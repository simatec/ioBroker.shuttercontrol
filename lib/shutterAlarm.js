        ...
        for (const i in result) {
            for (const s in shutterSettings) {
                if (shutterSettings[s].shutterName == result[i].shutterName) {
                    ...
                    const _alarmWind1 = adapter.config.alarmWind1 != '' ? await adapter.getForeignStateAsync(adapter.config.alarmWind1)
                        .catch((e) => adapter.log.warn(e)) : null;
                    const statusAlarmWind1 = _alarmWind1?.val ? _alarmWind1.val : false;

                    const _alarmWind2 = adapter.config.alarmWind2 != '' ? await adapter.getForeignStateAsync(adapter.config.alarmWind2)
                        .catch((e) => adapter.log.warn(e)) : null;
                    const statusAlarmWind2 = _alarmWind2?.val ? _alarmWind2.val : false;

                    const _alarmRain = adapter.config.alarmRain != '' ? await adapter.getForeignStateAsync(adapter.config.alarmRain)
                        .catch((e) => adapter.log.warn(e)) : null;
                    const statusAlarmRain = _alarmRain?.val ? _alarmRain.val : false;

                    const _alarmFrost = adapter.config.alarmFrost != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFrost)
                        .catch((e) => adapter.log.warn(e)) : null;
                    const statusAlarmFrost = _alarmFrost?.val ? _alarmFrost.val : false;

                    const _alarmFire = adapter.config.alarmFire != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFire)
                        .catch((e) => adapter.log.warn(e)) : null;
                    const statusAlarmFire = _alarmFire?.val ? _alarmFire.val : false;


                    const _triggerState = shutterSettings[s].triggerID != '' ? await adapter.getForeignStateAsync(shutterSettings[s].triggerID)
                        .catch((e) => adapter.log.warn(e)) : null;
...
async function checkFrostAlarm(adapter, Shutter) {
    return new Promise(async (resolve) => {
        ...
        const _resAlarmFrost = adapter.config.alarmFrost != '' ? await adapter.getForeignStateAsync(adapter.config.alarmFrost)
            .catch((e) => adapter.log.warn(`check frost alarm is not possible: ${e}`)) : null;
        const resAlarmFrost = _resAlarmFrost?.val ? _resAlarmFrost.val : false;
