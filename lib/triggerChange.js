    ...
    for (const i in arrayChangeTrigger) {
        for (const s in shutterSettings) {
            if (shutterSettings[s].shutterName == arrayChangeTrigger[i].shutterName) {
                setTimeout(function () {
                    if (shutterSettings[s].triggerChange == 'onlyUp' || shutterSettings[s].triggerChange == 'upDown' || (shutterSettings[s].triggerChange == 'off' && shutterSettings[s].autoDrive != 'off' && shutterSettings[s].driveAfterClose == true)) {
                    ...
                    if (shutterSettings[s].triggerChange == 'off' && shutterSettings[s].autoDrive != 'off' && shutterSettings[s].driveAfterClose == true) {
                        let nameDevice = shutterSettings[s].shutterName.replace(/[.;, ]/g, '_');
                        /**
                         * @param {any} err
                         * @param {boolean} state
