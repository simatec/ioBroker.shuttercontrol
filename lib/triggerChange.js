...
function triggerChange(resTriggerChange, adapter) {
    ...
    for (const i in arrayChangeTrigger) {
        setTimeout(function () {
            if (arrayChangeTrigger[i].triggerChange == 'onlyUp' || arrayChangeTrigger[i].triggerChange == 'upDown' || (arrayChangeTrigger[i].triggerChange == 'off' && arrayChangeTrigger[i].autoDrive != 'off') ) {
