...
async function sunProtect(adapter, elevation, azimuth, shutterSettings) {
    const driveDelayUpSleep = adapter.config.driveDelayUpAstro != 0 ? adapter.config.driveDelayUpAstro * 1000 : 2000;
            ...
            for (const i in result) {
                for (const s in shutterSettings) {
                    if (shutterSettings[s].shutterName == result[i].shutterName) {
                        ...
                        const _autoSunState = await adapter.getStateAsync(`shutters.autoSun.${nameDevice}`);

                        if (_autoSunState && _autoSunState === true || _autoSunState && _autoSunState.val === true) {
                            let currentValue = '';
                            let _triggerState;
                            let mustValue = '';
                            let mustValueTilted = '';
                            ...
                            switch (shutterSettings[s].type) {
                                ...
                                case 'in- & outside temperature': // in- & outside temperature
                                    _triggerState = shutterSettings[s].triggerID != '' ? await adapter.getForeignStateAsync(shutterSettings[s].triggerID) : null;
                                    ...
                                    if ((currentValue === mustValue || currentValue === mustValueTilted) && shutterSettings[s].tempSensor != '' || (currentValue != mustValue && currentValue != mustValueTilted && shutterSettings[s].autoDrive != 'off' && shutterSettings[s].tempSensor != '') || (shutterSettings[s].triggerID == '' && shutterSettings[s].tempSensor != '')) {
                                        ...
                                        const _insideTempState = shutterSettings[s].tempSensor != '' ? await adapter.getForeignStateAsync(shutterSettings[s].tempSensor) : null;
                                        if (typeof _insideTempState != undefined && _insideTempState != null) {
                                            insideTemp = parseFloat(_insideTempState.val);
                                        }

                                        const _outsideTempState = shutterSettings[s].outsideTempSensor != '' ? await adapter.getForeignStateAsync(shutterSettings[s].outsideTempSensor) : null;
                                        if (typeof _outsideTempState != undefined && _outsideTempState != null) {
                                            outsideTemp = parseFloat(_outsideTempState.val);
                                        }

                                        const _sunLight = shutterSettings[s].lightSensor != '' ? await adapter.getForeignStateAsync(shutterSettings[s].lightSensor) : null;
                                        if (typeof _sunLight != undefined && _sunLight != null) {
