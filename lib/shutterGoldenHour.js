...
async function shutterGoldenHour(adapter, goldenHourEnd, goldenHour, shutterSettings) {
    ...
    if (goldenHourEnd) {
        ...
        const upGoldenHour = schedule.scheduleJob('shutterUpGoldenHourEnd', `${upTime[1]} ${upTime[0]} * * *`, async function () {
            if (shutterSettings) {
                const resLiving = shutterSettings.filter((d) => d.typeUp == 'goldenhour End'); // Filter Area goldenhour end
    ...
    if (goldenHour) {
        ...
        const downGoldenHour = schedule.scheduleJob('shutterDownGoldenHour', `${upTime[1]} ${upTime[0]} * * *`, async function () {
            if (shutterSettings) {
                const resLiving = shutterSettings.filter((d) => d.typeDown == 'goldenhour'); // Filter Area Goldenhour
                const result = resLiving.filter((d) => d.enabled === true || d.enabled === 'true'); // Filter enabled
