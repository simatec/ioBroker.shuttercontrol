                                ...
                                if (currentValue === mustValue || (currentValue != mustValue && result[i].autoDrive != 'onlyDown' && result[i].autoDrive != 'off')) {
                                    ...
                                    adapter.getForeignState(result[i].name, (err, state) => {
                                        if (typeof state != undefined && state != null && state.val != shutterHeight && Date.now() >= (state.lc + noGoDelay)) {
                                            ...
                                            result[i].currentHeight = shutterHeight;
                                            adapter.log.debug('shutterUpLiving #1 ' + result[i].shutterName + ' already up at: ' + shutterHeight + '% - setting current action: ' + result[i].currentAction);
                                            shutterState(result[i].name, adapter);
										}
                                    });
                                    ...
                                    adapter.getForeignState(result[i].name, (err, state) => {
                                        if (typeof state != undefined && state != null && state.val != shutterHeight  && Date.now() >= (state.lc + noGoDelay)) {
                                            ...
                                            result[i].currentHeight = shutterHeight;
                                            adapter.log.debug('shutterUpLiving #2 ' + result[i].shutterName + ' already up at: ' + shutterHeight + '% - setting current action: ' + result[i].currentAction);
                                            shutterState(result[i].name, adapter);
										}
                                    });
                                         ...
                                         */
                                    adapter.getForeignState(result[i].name, (err, state) => {
                                        if (typeof state != undefined && state != null && state.val != result[i].shutterHeight) {
                                            adapter.log.info('#15 Will close ID: ' + result[i].shutterName + ' value: ' + shutterHeight + '%' + ' after the window has been closed ');
                                        ...
                                        if (currentValue === mustValue || (currentValue != mustValue && result[i].autoDrive != 'onlyDown' && result[i].autoDrive != 'off')) {
                                            ...
                                            adapter.getForeignState(result[i].name, (err, state) => {
                                                if (typeof state != undefined && state != null && state.val != shutterHeight) {
                                                    ...
                                                    result[i].currentHeight = shutterHeight;
                                                    adapter.setState('shutters.autoLevel.' + nameDevice, { val: result[i].currentHeight, ack: true });
                                                    adapter.log.debug('shutterUpLiving #3 '  + result[i].shutterName + ' already up at: ' + shutterHeight + '% - setting current action: ' + result[i].currentAction);
                                                    shutterState(result[i].name, adapter);
                                                }
                                            });
                                        } else if (result[i].triggerID == '') {
                                            ...
                                            adapter.getForeignState(result[i].name, (err, state) => {
                                                if (typeof state != undefined && state != null && state.val != shutterHeight) {
                                                    adapter.log.info('#17 Set ID: ' + result[i].shutterName + ' value: ' + shutterHeight + '%');
                                                    adapter.setForeignState(result[i].name, shutterHeight, false);
                                                    result[i].currentHeight = shutterHeight;
                                                    adapter.setState('shutters.autoLevel.' + nameDevice, { val: result[i].currentHeight, ack: true });
                                                    ...
                                                    result[i].currentHeight = shutterHeight;
                                                    adapter.setState('shutters.autoLevel.' + nameDevice, { val: result[i].currentHeight, ack: true });
                                                    adapter.log.debug('shutterUpLiving #4 '  + result[i].shutterName + ' already up at: ' + shutterHeight + '% - setting current action: ' + result[i].currentAction);
                                                    shutterState(result[i].name, adapter);
                                                }
                                            });
                                        } else if (currentValue != mustValue && result[i].autoDrive == 'onlyDown' && result[i].driveAfterClose == true) {
											...
											adapter.getForeignState(result[i].name, (err, state) => {
												if (typeof state != undefined && state != null && state.val != result[i].shutterHeight) {
													adapter.log.info('#18 Will close ID: ' + result[i].shutterName + ' value: ' + shutterHeight + '%' + ' after the window has been closed ');
													result[i].triggerHeight = shutterHeight;
													adapter.log.debug('save new trigger height: ' + shutterHeight + '%');
													result[i].triggerAction = result[i].currentAction;
													adapter.log.debug('save new trigger action: ' + result[i].triggerAction);
