...
async function buttonAction(adapter, buttonState, shutterSettings) {
    ...
    if (resultFull) {
        switch (buttonState) {
            case 'openLiving':
                resLiving = resultFull.filter((/** @type {{ typeUp: string; }} */ d) => d.typeUp == 'living' || d.typeUp == 'living-auto');
                break;
            case 'closeLiving':
                resLiving = resultFull.filter((/** @type {{ typeDown: string; typeUp: string; }} */ d) => d.typeDown == 'living' || d.typeUp == 'living-auto');
                break;
            case 'openSleep':
                resLiving = resultFull.filter((/** @type {{ typeUp: string; }} */ d) => d.typeUp == 'sleep' || d.typeUp == 'sleep-auto');
                break;
            case 'closeSleep':
                resLiving = resultFull.filter((/** @type {{ typeDown: string; typeUp: string; }} */ d) => d.typeDown == 'sleep' || d.typeUp == 'sleep-auto');
                break;
            case 'openChildren':
                resLiving = resultFull.filter((/** @type {{ typeUp: string; }} */ d) => d.typeUp == 'children' || d.typeUp == 'children-auto');
                break;
            case 'closeChildren':
                resLiving = resultFull.filter((/** @type {{ typeDown: string; typeUp: string; }} */ d) => d.typeDown == 'children' || d.typeUp == 'children-auto');
                break;
            case 'openAll':
                resLiving = resultFull.filter((/** @type {{ typeUp: string; }} */ d) => d.typeUp != 'manual-only');
                break;
            case 'closeAll':
                resLiving = resultFull.filter((/** @type {{ typeDown: string; }} */ d) => d.typeDown != 'manual-only');
                break;
            case 'sunProtect':
                resLiving = resultFull.filter((/** @type {{ typeDown: string; }} */ d) => d.typeDown != 'manual-only');
                break;
        ...
        let resEnabled = resLiving.filter((/** @type {{ enabled: boolean; }} */ d) => d.enabled === true); // Filter enabled
        ...
        for (const i in result) {
            for (const s in shutterSettings) {
                if (shutterSettings[s].shutterName == result[i].shutterName) {
                    ...
                    checkFrostAlarm(adapter, shutterSettings[s], function (/** @type {boolean} */ resAlarmFrost) {
                        ...
                        setTimeout(async function () {
                            if (buttonState == 'closeAll' || buttonState == 'closeLiving' || buttonState == 'closeSleep' || buttonState == 'closeChildren') {
                                ...
                                if (typeof _shutterState != undefined && _shutterState != null && _shutterState.val) {
