...
async function shutterDownChildren(adapter, downTimeChildren, delayDown, autoChildrenStr, shutterSettings) {
    const driveDelayUpChildren = adapter.config.driveDelayUpChildren != 0 ? adapter.config.driveDelayUpChildren * 1000 : 2000;
    ...
    try {
        const downChildren = schedule.scheduleJob('shutterDownChildren', downTime[1] + ' ' + downTime[0] + ' * * *', async function () {
            ...
            setTimeout(async function () {
                if (shutterSettings) {
                    const resChildren = shutterSettings.filter((/** @type {{ typeDown: string; }} */ d) => d.typeDown == 'children'); // Filter Area children
                    const result = resChildren.filter(d => d.enabled === true); // Filter enabled
                    let number = 0;
                    ...
                    for (const i in result) {
                        for (const s in shutterSettings) {
                            if (shutterSettings[s].shutterName == result[i].shutterName) {
                                ...
                                const statusAlarmFrost = checkFrostAlarm(adapter, shutterSettings[s]);
                ...
                if (autoChildrenStr == true) {
                    setTimeout(async function () {
                        if (shutterSettings) {
                            const resChildren = shutterSettings.filter((/** @type {{ typeDown: string; }} */ d) => d.typeDown == 'children-auto'); // Filter Area children
                            ...
                            for (const i in result) {
                                for (const s in shutterSettings) {
                                    if (shutterSettings[s].shutterName == result[i].shutterName) {
                                        ...
                                        // overwrite target position and downAction if frost alarm is set.
                                        if (statusAlarmFrost && shutterSettings[s].enableAlarmFrost == true) {
                                                    ...
                                                    setTimeout(async function () {
                                                        ...
                                                        if (currentValue === mustValue || currentValue === mustValueTilted || (currentValue != mustValue && currentValue != mustValueTilted && shutterSettings[s].autoDrive != 'onlyUp' && shutterSettings[s].autoDrive != 'off')) {
                                                            ...
                                                            if (typeof _shutterState != undefined && _shutterState != null && _shutterState.val != targetLevel2Set) {
                                                                ...
                                                                await setShutterState(adapter, shutterSettings, shutterSettings[s], targetLevel2Set, nameDevice, 'Children down #29');
                                                            ...
                                                            if (typeof _shutterState != undefined && _shutterState != null && _shutterState.val != targetLevel2Set) {
                                                                ...
                                                                await setShutterState(adapter, shutterSettings, shutterSettings[s], targetLevel2Set, nameDevice, 'Children down #30');
