...
async function elevationDown(adapter, elevation, azimuth, shutterSettings) {
    ...
    if (resultFull) {
        ...
        const resLiving = resultFull.filter((/** @type {{ typeDown: string; }} */ d) => d.typeDown == 'elevation');
        const resEnabled = resLiving.filter((/** @type {{ enabled: boolean; }} */ d) => d.enabled === true);
        ...
        for (const i in result) {
            for (const s in shutterSettings) {
                if (shutterSettings[s].shutterName == result[i].shutterName) {
                    ...
                    // @ts-ignore
                    checkPendingAlarm(adapter, shutterSettings[s], async function (resAlarmPending) {
                        pendingAlarm = resAlarmPending;
                        let statusAlarmFrost = false;

                        // @ts-ignore
                        checkFrostAlarm(adapter, shutterSettings[s], async function (resAlarmFrost) {
                                        ...
                                        setTimeout(async function () {
                                            ...
                                            if (currentValue === mustValue || currentValue === mustValueTilted || (currentValue != mustValue && currentValue != mustValueTilted && shutterSettings[s].autoDrive != 'onlyUp' && shutterSettings[s].autoDrive != 'off')) {
                                                ...
                                                if (typeof _shutterState != undefined && _shutterState != null && _shutterState.val != targetLevel2Set) {
                                                    await setShutterState(adapter, shutterSettings[s], targetLevel2Set, nameDevice, 'Elevation Down #3');
                                                    ...
                                                    await setShutterInfo(adapter, shutterSettings[s], nameDevice);
                                                ...
                                                if (typeof _shutterState != undefined && _shutterState != null && _shutterState.val != targetLevel2Set) {
                                                    await setShutterState(adapter, shutterSettings[s], targetLevel2Set, nameDevice, 'Elevation Down #4');
