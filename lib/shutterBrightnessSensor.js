...
// @ts-ignore
function shutterBrightnessSensor(adapter, brightnessValue, shutterSettings, brightnessDown) {
                                                        ...
                                                        adapter.getForeignState(shutterSettings[s].triggerID, (err, state) => {
                                                            ...
                                                            if (currentValue === mustValue || currentValue === mustValueTilted || (currentValue != mustValue && currentValue != mustValueTilted && shutterSettings[s].autoDrive != 'onlyUp' && shutterSettings[s].autoDrive != 'off')) {
                                                                ...
                                                                adapter.getForeignState(shutterSettings[s].name, (err, state) => {
                                                                    if (typeof state != undefined && state != null && state.val != targetLevel2Set /*&& shutterSettings[s].lastAutoAction != 'Down_Brightness' && (state.val == shutterSettings[s].heightDownSun || state.val == shutterSettings[s].heightUp || state.val == shutterSettings[s].triggerDrive)*/ && shutterSettings[s].triggerAction != 'down') {
                                                                    ...
                                                                    else if (typeof state != undefined && state != null && state.val == targetLevel2Set && (shutterSettings[s].currentHeight != targetLevel2Set || shutterSettings[s].currentAction != downAction)) {
                                                                        shutterSettings[s].currentHeight = targetLevel2Set;
                                                                        shutterSettings[s].currentAction = downAction;
                                                                        adapter.setState('shutters.autoState.' + nameDevice, { val: shutterSettings[s].currentAction, ack: true });
                                                                ...
                                                                // @ts-ignore
                                                                adapter.getForeignState(shutterSettings[s].name, (err, state) => {
                                                                    if (typeof state != undefined && state != null && state.val != targetLevel2Set && (state.val == shutterSettings[s].heightDownSun || state.val == shutterSettings[s].heightUp || state.val == shutterSettings[s].triggerDrive)) {
