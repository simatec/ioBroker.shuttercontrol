...
async function translateYandex(text, targetLang, apiKey) {
    ...
    try {
        const url = `https://translate.yandex.net/api/v1.5/tr.json/translate?text=${encodeURIComponent(text)}&lang=en-${targetLang}`;
        // @ts-ignore
        const response = await axios({url, timeout: 15000, headers: {'Authorization': `Api-Key ${apiKey}`}});
